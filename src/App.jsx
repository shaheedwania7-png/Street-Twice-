...
function Login({onNav,onLogin}){
  ...
  async function submit(){
    ...
    setLoading(true);setErr("");
    try{await sbAuth(email,pw,"login");const user=await sbGetUser();if(!user||!user.id)throw new Error("Login failed — please try again.");const rows=await sbFetch(`players?id=eq.${user.id}&limit=1`);if(!rows?.[0])throw new Error("Profile not found. Please sign up.");onLogin(rows[0]);onNav("home");}
    catch(e){setErr(e.message);}
    setLoading(false);
  }
  return <div style={{minHeight:"100vh",background:C.bg,padding:"24px 20px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
...
export default function App(){
  ...
  useEffect(()=>{
    sbGetUser().then(async user=>{
      if(user){
        try{if(user&&user.id){const rows=await sbFetch(`players?id=eq.${user.id}&limit=1`);if(rows?.[0]){setPlayer(rows[0]);setScreen("app");}}}
        catch(_){}
      }
      setAuthReady(true);
  ...
  // Safety redirect — if app screen loads but player is null, go to landing
  if(screen==="app"&&!player){
    setTimeout(()=>setScreen("landing"),0);
    return <div style={{minHeight:"100vh",background:"#060608",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{fontSize:10,color:"#555",letterSpacing:4,textTransform:"uppercase"}}>Loading...</div></div>;
  }
