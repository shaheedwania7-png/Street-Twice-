...
const SOLO_TESTS = [
  // ── PACE — 4 sub-tests ──────────────────────
  { id:"sprint_explosive", stat:"pace", icon:"💥", label:"Explosive Sprint (10m)",  camMode:"sprint",
    subStat:"explosive_pace",
    desc:"10m sprint from a standing start. Measures pure acceleration and first-step quickness — the most important pace stat in tight spaces.",
    steps:["Measure exactly 10m — place a cone at each end","Phone sideways on the ground — BOTH cones fully visible in frame","Stand at the start cone completely still","Press Start Sprint, then explode off the line as fast as possible","3 attempts — best time counts"],
    scoring:"Motion detection auto-times you. Compared against your age group. Under 1.7s = elite explosive pace." },

  { id:"sprint_speed", stat:"pace", icon:"⚡", label:"Top Speed Sprint (30m)", camMode:"sprint",
    subStat:"top_speed",
    desc:"30m sprint with a rolling start to measure pure top-end speed — how fast you go once you are already moving.",
    steps:["Measure 30m — cones at each end","Film from the side — both cones visible","Take a 5m run-up before the start cone — this removes acceleration from the test","Sprint through as fast as possible to the finish cone","3 attempts — best counts"],
    scoring:"Auto-timed. Measures your top velocity. Under 3.5s for 30m = elite top speed." },

  { id:"sprint_agility_run", stat:"pace", icon:"🔀", label:"Agility Speed (5-10-5 Drill)", camMode:"record",
    subStat:"agility_speed",
    desc:"5-10-5 shuttle drill — sprint 5m right, 10m left, 5m back. Measures change-of-direction speed under full sprint conditions.",
    steps:["Mark 3 cones: centre, 5m right, 5m left","Film from above or wide front angle","Sprint 5m right, touch cone, sprint 10m to left cone, touch it, sprint 5m back to centre","3 attempts — best time counts","Every touch must be genuine — foot must cross the cone line"],
    scoring:"Enter your best time in seconds. Under 4.2s = elite. Compares direction speed to top speed ratio." },

  { id:"sprint_endurance", stat:"pace", icon:"♾️", label:"Sprint Endurance (8×20m)", camMode:"record",
    subStat:"endurance",
    desc:"8 repeated 20m sprints with only 20 seconds rest between each. Measures how well your sprint speed holds up under fatigue — crucial for real game fitness.",
    steps:["Set up 20m course as normal","Film from the side — both cones visible throughout","Sprint 20m, walk back in 20 seconds, sprint again — 8 times total","Film all 8 in one continuous take","Time each sprint — write them down"],
    scoring:"Enter your first sprint time and your last sprint time. The drop-off % is your endurance score. Less than 8% drop = elite stamina." },

  { id:"sprint_reaction", stat:"pace", icon:"🚦", label:"Reaction Time Sprint", camMode:"record",
    subStat:"reaction",
    desc:"A partner (or audio cue) gives a random start signal. Measures how quickly you react and accelerate from a standing start — pure mental and physical quickness.",
    steps:["Set up 10m course — phone films from the side","Stand at start in a ready position — weight slightly forward","Partner claps or shouts GO at a random moment (no counting down)","Sprint 10m the instant you hear it","5 attempts — your reaction time is estimated from video"],
    scoring:"Enter your 5 times. The average is your reaction sprint score. Under 1.65s = elite reaction." },
  ...
  { id:"jump", stat:"jumping", icon:"⬆️", label:"Jump Height", camMode:"jump",
    ...
    steps:["Stand sideways to camera — full body visible head to feet","Jump straight up as high as you can","Camera detects when feet leave and land","5 jumps — best is saved"] },

  { id:"pushups", stat:"physical", icon:"💪", label:"Push-up Counter", camMode:"pushups",
    desc:"Camera counts your reps automatically.",
    steps:["Phone on the ground 2m away, pointing sideways","Full body must be visible","Chest must touch the floor every rep","Camera counts — do as many as you can"] },
  ...
  { id:"agility", stat:"agility", icon:"🏃", label:"Agility Drill", camMode:"agility",
    ...
    steps:["4 cones around you — front, back, left, right — 2m away","Stand centre facing your phone","React to direction commands","10 commands — fastest average reaction wins"] },

  { id:"shooting", stat:"shooting", icon:"🎯", label:"Shooting", camMode:"record",
    desc:"Shoot at a marked target. Count hits and enter your score.",
    steps:["Mark a 60×60cm target on wall with tape","Stand 10m back — film from the side","10 shots at the target","Count hits and enter your score"] },

  { id:"passing", stat:"passing", icon:"🔄", label:"Passing Accuracy", camMode:"record",
    desc:"Pass at targets from set distances. Accuracy and weight both scored.",
    steps:["Mark a 50cm target on a wall","10 passes from 10m — film from behind you","Count hits — repeat at 15m and 20m"] },
...
function CameraView({ mode = "record", onResult, instructions = [], timerSecs = 60, playerAge = 22 }) {
  ...
  useMotion(cam.videoRef, cam.canvasRef, phase === "live" && cam.ready, (score) => {
    ...
    if (mode === "pushups") {
      ...
      const r = motionBuf.current.slice(-8), p2 = motionBuf.current.slice(-16, -8);
      const ar = r.reduce((a,b) => a+b.score, 0) / r.length;
      const ap = p2.length ? p2.reduce((a,b) => a+b.score, 0) / p2.length : 0;
  ...
  async function flipCamera() {
    ...
    setFacingMode(nf);
    cam.stopCam();
    setTimeout(() => cam.startCam(nf), 300);
  }

  function stopAndScore() {
    cam.recStop(); timer.stop();
    if (mode === "sprint" && sprintMs) {
      onResult({ score: sprintToScore(sprintMs, group), label: `${sprintTime}s (avg: ${SPRINT_BENCH[group].avg}s)` });
    } else if (mode === "pushups") {
      onResult({ score: rawToScore(pushCount, PUSH_BENCH[group]), label: `${pushCount} reps` });
    } else if (mode === "jump" && jumpHang) {
      const h = parseFloat(jumpHang), cm = Math.round(1.22*h*h*9.81*100);
      onResult({ score: rawToScore(cm, JUMP_BENCH[group]), label: `${cm}cm` });
    } else {
      setPhase("scoring");
    }
  }

  const oc = mode === "sprint" ? C.green : mode === "pushups" ? C.red : mode === "jump" ? C.cyan : C.blue;

  // ── SETUP SCREEN ──────────────────────────────
  if (phase === "setup") return (
    <div style={{ background:"#0A0A18", borderRadius:14, overflow:"hidden", border:`2px solid ${oc}33` }}>
      <div style={{ padding:"20px 18px" }}>
        <div style={{ background:`${C.blue}18`, border:`1.5px solid ${C.blue}33`, borderRadius:10, padding:14, marginBottom:16 }}>
          <div style={{ fontSize:12, color:C.blue, letterSpacing:2, textTransform:"uppercase", fontWeight:700, marginBottom:8 }}>Setup Steps</div>
          {instructions.map((s, i) => (
            <div key={i} style={{ display:"flex", gap:12, marginBottom:10 }}>
              <div style={{ width:26, height:26, borderRadius:"50%", background:`${oc}22`, border:`2px solid ${oc}55`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:800, color:oc, flexShrink:0 }}>{i+1}</div>
  ...
  // ── LIVE CAMERA — true fullscreen overlay ──────
      ...
      <div style={{ flex:1, position:"relative", overflow:"hidden", minHeight:0 }}>
        ...
        {!cam.ready && !cam.err && (
          <div style={{ position:"absolute", inset:0, background:"#000", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:16 }}>
            ...
            <div style={{ fontSize:18, color:C.ghost }}>Starting camera...</div>
          </div>
        )}

        {/* Error overlay */}
        {cam.err && (
          <div style={{ position:"absolute", inset:0, background:"#000000EE", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:18, padding:28, textAlign:"center" }}>
            <div style={{ fontSize:48 }}>⚠️</div>
            <div style={{ fontSize:16, color:C.red, lineHeight:1.7, whiteSpace:"pre-line" }}>{cam.err}</div>
            <Btn sm v="ghost" onClick={() => { cam.stopCam(); setPhase("setup"); }}>← Go Back</Btn>
          </div>
        )}

        {/* HUD overlays when camera is live */}
        {cam.ready && <>
          {/* Sprint lines */}
          {mode === "sprint" && <>
            ...
            <div style={{ position:"absolute", top:20, left:"50%", transform:"translateX(-50%)", background:"#000000CC", borderRadius:14, padding:"12px 22px", textAlign:"center", minWidth:220 }}>
              {sprintState==="idle"    && <div style={{ fontSize:16, color:C.green, fontWeight:700 }}>Press START below ↓</div>}
              {sprintState==="waiting" && <div style={{ fontSize:20, fontWeight:900, color:C.green }}>⚡ RUN NOW!</div>}
              {sprintState==="running" && <div style={{ fontSize:18, fontWeight:900, color:C.gold }}>🏃 SPRINTING...</div>}
              {sprintState==="done"    && <div><div style={{ fontSize:28, fontWeight:900, color:C.green }}>{sprintTime}s ✓</div><div style={{ fontSize:13, color:C.ghost }}>avg {SPRINT_BENCH[group].avg}s for {AGE_LABELS[group]}</div></div>}
            </div>
          </>}
          ...
          {mode === "pushups" && (
            <div style={{ position:"absolute", top:20, right:20, background:"#000000CC", borderRadius:16, padding:"14px 20px", textAlign:"center" }}>
              <div style={{ fontSize:52, fontWeight:900, color:C.green, lineHeight:1 }}>{pushCount}</div>
          ...
          {/* Jump */}
          {mode === "jump" && (
            <div style={{ position:"absolute", top:20, left:"50%", transform:"translateX(-50%)", background:"#000000CC", borderRadius:14, padding:"12px 22px", textAlign:"center" }}>
              {jumpHang
                ? <div style={{ fontSize:26, fontWeight:900, color:C.green }}>{Math.round(1.22*parseFloat(jumpHang)*parseFloat(jumpHang)*9.81*100)}cm ✓</div>
                : <div style={{ fontSize:16, color:airborne?C.gold:C.ghost }}>{airborne?"⬆️ AIRBORNE...":"Jump when ready"}</div>}
            </div>
          )}
          ...
          {timer.on && (
            <div style={{ position:"absolute", top:20, left:20, background:"#000000CC", borderRadius:10, padding:"8px 16px" }}>
              <div style={{ fontSize:24, fontWeight:900, color:timer.t<10?C.red:C.white }}>{timer.display}</div>
            </div>
          )}
          ...
          {cam.recording && (
            <div style={{ position:"absolute", bottom:20, left:20, display:"flex", alignItems:"center", gap:8, background:"#000000AA", borderRadius:8, padding:"8px 14px" }}>
              ...
              <span style={{ fontSize:15, color:C.white, fontWeight:700 }}>RECORDING</span>
            </div>
          )}

          {/* Flip camera */}
          <button onClick={flipCamera} style={{ position:"absolute", top:20, right:20, background:"#000000AA", border:"1.5px solid #333355", color:C.white, borderRadius:10, padding:"10px 14px", cursor:"pointer", fontSize:22 }}>🔄</button>
        </>}
      </div>

      {/* CONTROLS BAR — always visible at bottom */}
      <div style={{ background:"#0A0A18", borderTop:`2px solid #1A1A2E`, padding:"16px 20px", display:"flex", gap:12, flexShrink:0 }}>
        {/* Sprint controls */}
        {mode === "sprint" && !cam.recording && sprintState === "idle" && (
          <Btn full v="green" onClick={() => { cam.recStart(); setSprintState("waiting"); timer.start(); }} style={{ fontSize:18, padding:"18px" }}>
            ⚡ Start Sprint
          </Btn>
        )}
        {mode === "sprint" && cam.recording && sprintState !== "done" && (
          <Btn full v="ghost" onClick={() => { cam.recStop(); timer.stop(); setSprintState("idle"); }} style={{ fontSize:18, padding:"18px" }}>
            ↩ Reset
          </Btn>
        )}
        {mode === "sprint" && sprintState === "done" && (
          <Btn full v="green" onClick={stopAndScore} style={{ fontSize:18, padding:"18px" }}>
            ✓ Save — {sprintTime}s
          </Btn>
        )}

        {/* Non-sprint controls */}
        {mode !== "sprint" && !cam.recording && (
          <Btn full v="green" onClick={() => { cam.recStart(); timer.start(); }} style={{ fontSize:18, padding:"18px" }}>
            ⏺ Start Recording
          </Btn>
        )}
        {mode !== "sprint" && cam.recording && (
          <Btn full v="red" onClick={stopAndScore} style={{ fontSize:18, padding:"18px" }}>
            ⏹ Stop & Save
          </Btn>
        )}

        {/* Always visible close button */}
        <button onClick={() => { cam.stopCam(); timer.stop(); setPhase("setup"); }}
          style={{ background:"#1A1A2E", border:"2px solid #333355", color:C.white, borderRadius:12, padding:"0 18px", cursor:"pointer", fontSize:15, fontWeight:700, flexShrink:0 }}>
          ✕
        </button>
      </div>
  ...
  // ── SCORING (for record mode) ──────────────────
  if (phase === "scoring") return (
    <div style={{ background:"#0A0A18", borderRadius:14, padding:22, border:`2px solid ${oc}33` }}>
      <div style={{ fontSize:20, fontWeight:900, marginBottom:10, color:C.white }}>Enter Your Score</div>
      <p style={{ color:C.ghost, fontSize:14, lineHeight:1.7, marginBottom:18 }}>Watch your clip back and enter your honest result. Clip uploaded for community verification.</p>
      <Input label="Your Score (0–99)" type="number" value={scoreInput} onChange={setScoreInput} placeholder="e.g. 65" />
      <Btn full onClick={() => { if(scoreInput) onResult({ score:parseInt(scoreInput), label:`${scoreInput}/99` }); }} disabled={!scoreInput} v="green" style={{ fontSize:17 }}>✓ Submit Score</Btn>
    </div>
  );
...
function DevelopTab({ player, onStat }) {
  ...
  function handleTestResult(res) {
    if (res.score != null && selTest?.stat) {
      onStat(selTest.stat, Math.min(99, Math.max(15, Math.round(res.score))));
  ...
  function goBack() {
    if (selTest) { setSelTest(null); setPhase("intro"); return; }
    if (view !== "menu") { setView("menu"); setChallengePhase("link"); return; }
  }

  return (
    <div style={{ minHeight:"100vh", background:C.bg, color:C.white, paddingBottom:100 }}>
      <Hdr
        title={selTest ? selTest.label : view==="menu" ? "Develop" : view==="tests" ? "Solo Tests" : "Challenges"}
        sub="Level Up"
        back={view!=="menu"||selTest ? goBack : undefined}
      />
      ...
      <div style={{ padding:"20px 20px", maxWidth:480, margin:"0 auto" }}>
          ...
          <>
            <div style={{ background:"#0A0A18", borderRadius:14, padding:18, marginBottom:20, border:"1px solid #1A1A2E" }}>
              ...
              {Object.entries(STATS_META).map(([k,m]) => (
                <StatBar key={k} label={m.l} value={player?.stats?.[k]} color={m.color}/>
              ))}
            </div>
            {[
              { id:"tests", icon:"🧪", label:"Solo Tests", desc:"Camera-based assessments — sprint timing, jump height, push-up counter, agility and more", color:C.green },
              ...
              <button key={m.id} onClick={() => setView(m.id)}
                style={{ width:"100%", background:`linear-gradient(135deg,${m.color}14,${m.color}06)`, border:`2px solid ${m.color}33`, borderRadius:16, padding:20, textAlign:"left", cursor:"pointer", color:C.white, marginBottom:14, display:"flex", alignItems:"center", gap:16 }}>
        ...
        {view==="tests" && !selTest && (
          <>
            <div style={{ background:`${C.blue}14`, border:"1.5px solid #0066FF33", borderRadius:12, padding:14, marginBottom:18 }}>
              <div style={{ fontSize:14, color:C.blue, fontWeight:700, marginBottom:4 }}>📏 Age-calibrated benchmarks</div>
              <div style={{ fontSize:13, color:C.ghost }}>All scores measured against the average for <strong style={{ color:C.white }}>{age ? AGE_LABELS[group] : "your age group"}</strong>.</div>
            </div>

            {/* Pace group */}
            <div style={{ background:`${C.green}14`, border:`2px solid ${C.green}33`, borderRadius:14, padding:16, marginBottom:14 }}>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
                <div style={{ fontSize:28 }}>⚡</div>
                <div>
                  <div style={{ fontSize:18, fontWeight:900, color:C.green }}>Pace & Speed</div>
                  <div style={{ fontSize:13, color:C.ghost }}>5 tests — explosive, top speed, agility run, endurance, reaction</div>
                </div>
              </div>
              {SOLO_TESTS.filter(t => t.subStat).map(test => {
                const subLabels = { explosive_pace:"Explosive", top_speed:"Top Speed", agility_speed:"Agility Run", endurance:"Endurance", reaction:"Reaction" };
                return (
                  <button key={test.id} onClick={() => { setSelTest(test); setPhase("intro"); }}
                    style={{ width:"100%", background:"#0A0A18", border:`1.5px solid ${C.green}22`, borderRadius:12, padding:14, textAlign:"left", cursor:"pointer", color:C.white, display:"flex", alignItems:"center", gap:14, marginBottom:8 }}>
                    <div style={{ fontSize:28 }}>{test.icon}</div>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:15, fontWeight:800 }}>{test.label}</div>
                      <div style={{ fontSize:12, color:C.ghost, marginTop:3 }}>
                        {test.camMode==="sprint" ? "🤖 Auto-timed" : "📷 Film + manual"} · <span style={{ color:C.green, fontWeight:700 }}>{subLabels[test.subStat]}</span>
                      </div>
                    </div>
            ...
            {/* Other skills */}
            <div style={{ fontSize:12, color:C.ghost, letterSpacing:2, textTransform:"uppercase", fontWeight:700, marginBottom:10 }}>Other Skills</div>
            {SOLO_TESTS.filter(t => !t.subStat).map(test => {
              const v = player?.stats?.[test.stat] || 0;
              return (
                <button key={test.id} onClick={() => { setSelTest(test); setPhase("intro"); }}
                  style={{ width:"100%", background:"#0A0A18", border:`2px solid ${v>0 ? C.green+"55" : "#1A1A2E"}`, borderRadius:14, padding:16, textAlign:"left", cursor:"pointer", color:C.white, display:"flex", alignItems:"center", gap:16, marginBottom:10 }}>
                  ...
                  <div>
                    {v > 0
                      ? <div style={{ fontSize:26, fontWeight:900, color:sc(v) }}>{v}</div>
                      : <div style={{ width:38, height:38, borderRadius:"50%", border:"2px solid #333355", display:"flex", alignItems:"center", justifyContent:"center" }}><div style={{ width:12, height:12, borderRadius:"50%", background:"#333355" }}/></div>}
                  </div>
                </button>
              );
            })}
          ...
          <>
            {phase==="intro" && (
              <>
                ...
                {age && selTest.camMode && selTest.camMode!=="record" && (
                  <div style={{ background:`${C.blue}14`, border:"1.5px solid #0066FF33", borderRadius:12, padding:14, marginBottom:16 }}>
                    <div style={{ fontSize:13, color:C.blue, fontWeight:700, marginBottom:4 }}>Your Age Group Benchmark</div>
                    {selTest.id==="sprint_explosive" && <div style={{ fontSize:15, color:C.white }}>Avg 10m: <strong style={{ color:C.green }}>{SPRINT_BENCH[group].avg ? (SPRINT_BENCH[group].avg * 0.5).toFixed(1) : "?"}s</strong></div>}
                    {selTest.id==="sprint_speed" && <div style={{ fontSize:15, color:C.white }}>Avg 30m: <strong style={{ color:C.green }}>{SPRINT_BENCH[group].avg}s</strong> · Elite: <strong style={{ color:C.gold }}>{SPRINT_BENCH[group].elite}s</strong></div>}
                    {selTest.id==="jump" && <div style={{ fontSize:15, color:C.white }}>Avg: <strong style={{ color:C.green }}>{JUMP_BENCH[group].avg}cm</strong> · Elite: <strong style={{ color:C.gold }}>{JUMP_BENCH[group].elite}cm</strong></div>}
                    {selTest.id==="pushups" && <div style={{ fontSize:15, color:C.white }}>Avg: <strong style={{ color:C.green }}>{PUSH_BENCH[group].avg} reps</strong> · Elite: <strong style={{ color:C.gold }}>{PUSH_BENCH[group].elite}</strong></div>}
                  </div>
                )}
                {selTest.steps?.length > 0 && (
                  <div style={{ background:"#0A0A18", borderRadius:12, padding:16, marginBottom:16, border:"1px solid #1A1A2E" }}>
                    <div style={{ fontSize:13, color:C.ghost, letterSpacing:2, textTransform:"uppercase", fontWeight:700, marginBottom:12 }}>How to Set Up</div>
                    {selTest.steps.map((s,i) => (
                      <div key={i} style={{ display:"flex", gap:12, marginBottom:10 }}>
                        <div style={{ width:26, height:26, borderRadius:"50%", background:`${C.green}22`, border:`2px solid ${C.green}55`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:800, color:C.green, flexShrink:0 }}>{i+1}</div>
                        <p style={{ margin:0, color:C.offwhite, fontSize:14, lineHeight:1.6 }}>{s}</p>
                      </div>
                    ))}
                  </div>
                ...
                {selTest.scoring && (
                  <div style={{ background:`${C.gold}0a`, border:"1.5px solid #FFB80033", borderRadius:10, padding:12, marginBottom:16 }}>
                    <div style={{ fontSize:13, color:C.gold, fontWeight:700, marginBottom:4 }}>⭐ Scoring</div>
                    <div style={{ fontSize:13, color:C.ghost, lineHeight:1.6 }}>{selTest.scoring}</div>
                  </div>
                )}
            ...
            {phase==="camera" && (
              selTest.camMode==="agility"
                ? <div style={{ background:"#0A0A18", borderRadius:14, padding:6, border:`2px solid ${C.gold}33` }}>
                    <AgilityDrill playerAge={age||22} onScore={scores => {
                      const avg = scores.reduce((a,b) => a+b, 0) / scores.length;
                      const s = avg<500?92:avg<700?82:avg<900?70:avg<1200?55:38;
                      onStat("agility", s);
                      showToast(`Agility score: ${s}/99 ✓`);
                      setPhase("done");
                    }}/>
                  </div>
                : <CameraView
                    mode={selTest.camMode}
                    onResult={handleTestResult}
                    instructions={selTest.steps||[]}
                    timerSecs={120}
                    playerAge={age||22}
                  />
            )}
            ...
            {phase==="done" && (
              <div style={{ textAlign:"center", padding:28 }}>
                ...
                <p style={{ color:C.ghost, fontSize:15, lineHeight:1.7, marginBottom:28 }}>Benchmarked against your age group. Keep testing to complete your card.</p>
            ...
            {challengePhase==="link" && (
              <>
                <div style={{ background:"#0A0A18", border:`2px solid ${C.blue}33`, borderRadius:14, padding:18, marginBottom:16 }}>
                  ...
                  <p style={{ color:C.ghost, fontSize:14, lineHeight:1.6, marginBottom:14 }}>Ask your opponent for their Challenge Code, then enter it below. Both players must be registered.</p>
                  {linked ? (
                    <div style={{ background:`${C.green}0a`, border:`2px solid ${C.green}22`, borderRadius:12, padding:14, display:"flex", gap:14, alignItems:"center" }}>
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        {age && calcAge(linked.date_of_birth) && Math.abs(age-calcAge(linked.date_of_birth))>=3 &&
                          <div style={{ fontSize:13, color:C.gold, fontWeight:700, marginTop:6 }}>⚡ Age gap — XP weighted</div>}
                      </div>
                ...
                {linked && (
                  <Btn full v="green" onClick={() => setChallengePhase("rounds")} style={{ fontSize:17 }}>Start Challenges →</Btn>
                )}
              </>
            )}
            ...
            {challengePhase==="rounds" && linked && (
              <>
                <div style={{ background:`${C.green}0a`, border:`2px solid ${C.green}22`, borderRadius:12, padding:14, marginBottom:16, display:"flex", gap:14, alignItems:"center" }}>
                  <Card player={linked} size="sm" glow={false}/>
                  <div>
                    <div style={{ fontSize:15, fontWeight:700, color:C.green }}>Playing vs</div>
                    <div style={{ fontSize:18, fontWeight:900 }}>{linked.name}</div>
                  </div>
                </div>
                <div style={{ background:"#0A0A18", borderRadius:14, padding:16, border:"1px solid #1A1A2E", marginBottom:14 }}>
                  ...
                  {[
                    { icon:"🦶", label:"Round 1–3", rule:"No slide tackles — positioning only", timer:180 },
                    { icon:"⚡", label:"Round 4–6", rule:"Slide tackles ONLY", timer:180 },
                    { icon:"👥", label:"Round 7–9", rule:"2v1 — two attackers", timer:180 },
                    { icon:"💨", label:"60s Blitz", rule:"Beat defender in 60 seconds", timer:60 },
                    { icon:"🌀", label:"Skill Round", rule:"Attacker must use a skill move", timer:120 },
                  ].map((r,i) => (
                ...
                <Btn full v="green" onClick={() => showToast("Challenge started! Both players record simultaneously.")} style={{ fontSize:17 }}>
                  Start Challenge Session
                </Btn>
                <div style={{ marginTop:14, background:`${C.blue}14`, border:"1.5px solid #0066FF33", borderRadius:12, padding:14 }}>
                  <div style={{ fontSize:14, color:C.blue, fontWeight:700, marginBottom:4 }}>🔒 Anti-Cheat</div>
                  <div style={{ fontSize:13, color:C.ghost, lineHeight:1.6 }}>Both players film simultaneously. Gap ≤2 = auto-averaged. 3–5 = pending. 6+ = frozen. Age gap XP applied.</div>
                </div>
              </>
            )}
          </>
