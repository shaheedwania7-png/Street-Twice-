...
function Login({ onNav, onLogin }) {
  ...
  async function submit() {
    ...
    try {
      ...
      // Try to get UID from auth response first
      let uid = authData?.user?.id || authData?.id;
      if (!uid) {
        // Wait a moment for token to settle then try getting user
        await new Promise(r => setTimeout(r, 500));
        const userObj = await sbGetUser();
        uid = userObj?.id;
      }
      if (!uid) throw new Error("Could not verify your account. Please check your email and password and try again.");
